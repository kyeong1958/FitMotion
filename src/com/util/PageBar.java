...
public class PageBar {
	 ...
	 * 페이지당 레코드 수 numPerPage (pageSize)
	 * 전체 레코드 수 totalRecord
	 * 현재 내가 바라보는 페이지 nowPage
	 * 내가 처리해야 할 페이지 이름 pagePath(xxx.jsp , xxx.mo)
	 * 
	 * 공식을 세우는데 필요한 인자
	 * 
	...
	public void setPageBar() {
		...
		if(totalRecord > 0) {
			//nowBlock이 0보다 클 때 처리하기
			//이전 페이지로 이동 해야 하므로 페이지 번호에 a태그를 붙여야 하고
			//pagePath뒤에 이동할 페이지 번호를 붙여서 호출 해야 함.
			if(nowBlock>0) {
			...
			//현재 블록에서 다음 블럭이 존재할 경우 이미지 추가하고 페이지 이동할 수 있도록
			//a태그 활용하여 링크 처리하기
